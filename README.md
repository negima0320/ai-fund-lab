...
backtestでは速度と再現性を優先し、原則としてGoogle News RSS取得とOpenAI / ChatGPT API呼び出しをOFFにします。各日付の処理では `calculate-indicators`、`market-context`、`screen`、`score`、`ai-decision`、`trade`、`reports/articles`、`db-save` の開始/終了と処理時間をコンソールに表示します。
