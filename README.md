...
現在は実売買を行いません。ローカルMac上で、ダミーデータを使った仮想売買シミュレーションを実装しています。
...
人間向けの日報やnote記事とは別に、後でChatGPTや別AIへ渡して改善案を分析するためのJSONLログを生成できます。保存先は `analysis_logs/<profile_id>/` で、`analysis_logs/` はGit管理しません。APIキー、秘密情報、raw news本文は含めません。
...
- `docs/ai-analysis-logs.md`: ChatGPT等へ渡すAI改善用JSONLログとサマリの設計
