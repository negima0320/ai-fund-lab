...
同じ期間で各profileのバックテストを実行すると、損切り方式、利確幅、RSI過熱フィルター、スコア閾値、出来高フィルターの違いを比較できます。`rookie_dealer_02_v2` と `rookie_dealer_02_v3` は過学習の可能性を含む仮説検証用profileです。
...
python src/main.py --mode compare-profiles --profiles rookie_dealer_02 rookie_dealer_02_v2 rookie_dealer_02_v3 rookie_dealer_03 --start-date YYYY-MM-DD --end-date YYYY-MM-DD
```
