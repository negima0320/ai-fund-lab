...
損切り方式の比較用に、2つのprofileを用意しています。

- `rookie_dealer_01`: `next_day_open` 方式。引け後に損切り判定し、翌営業日寄り付きで約定する現実寄りの検証
- `rookie_dealer_02`: `intraday_stop` 方式。当日安値が損切りラインに到達した場合、逆指値を想定して損切りライン価格で約定したものとして検証

同じ期間で両profileのバックテストを実行すると、損切り方式の違いを比較できます。
