"""Phase 12-B limited allocation strategy check.

This is a 2025-only lightweight strategy check for the Phase 12-A3 dynamic
allocation rule. It reads existing scored artifacts and does not run a full
backtest, change profiles, overwrite models, or regenerate historical
predictions. Future columns are used only for BUY quality reporting.
"""
...
REPORT_STEM = "phase12b_limited_allocation_strategy_check_2025"
...
STRATEGIES = [
    ...
    StrategySpec("S1_opportunity_top5_equal", rank_column="opportunity_proba", exit_guard=False),
...
class Phase12BLimitedAllocationStrategyCheck:
    def __init__(self, root: Path | str = ROOT, *, options: Phase12BOptions | None = None) -> None:
        self.root = Path(root)
        self.options = options or Phase12BOptions()

    def run(self) -> Phase12BPaths:
    ...
    def build_report(self) -> dict[str, Any]:
        ...
        comparison = self.strategy_comparison(strategy_results, buy_quality)
        report = {
            "metadata": self.metadata(),
            "conditions": self.conditions(),
            "dataset_summary": self.dataset_summary(data),
    ...
    def load_frame(self) -> pd.DataFrame:
        ...
        available = [column for column in required_or_useful if column in data.columns]
    ...
    def simulate(self, data: pd.DataFrame, spec: StrategySpec) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for current_date in dates:
            ...
            if spec.allocation_mode.startswith("dynamic"):
                ranked = ranked.copy()
                ...
                positions.append(
                    {
                        ...
                        "allocation_weight": _safe_float(row.get("a3_3_allocation_weight")) if spec.allocation_mode.startswith("dynamic") else 1.0,
    ...
    def strategy_comparison(self, rows: list[dict[str, Any]], quality: list[dict[str, Any]]) -> dict[str, Any]:
        by_strategy = {row["strategy"]: row for row in rows}
        by_quality = {row["strategy"]: row for row in quality}
        baseline = by_strategy.get("S0_baseline_equal_allocation", {})
        s2 = by_strategy.get("S2_opportunity_top5_E4", {})
        dynamic = [by_strategy.get("S3a_dynamic_raw_weight", {}), by_strategy.get("S3b_dynamic_normalized_weight", {})]
        improved_vs_baseline = [row.get("strategy") for row in dynamic if self.passes_phase12b(row, baseline)]
        improved_vs_s2 = [
            row.get("strategy")
            for row in dynamic
            if row
            and (_safe_float(row.get("DD")) or -1.0) > (_safe_float(s2.get("DD")) or 1.0)
            and (_safe_float(row.get("PF")) or 0.0) >= max(0.0, (_safe_float(s2.get("PF")) or 0.0) - 0.25)
            and (by_quality.get(row.get("strategy"), {}).get("downside_bad_rate") or 1.0) < (by_quality.get("S2_opportunity_top5_E4", {}).get("downside_bad_rate") or 0.0)
        ]
        best = self.best_strategy(rows)
        raw = by_strategy.get("S3a_dynamic_raw_weight", {})
        normalized = by_strategy.get("S3b_dynamic_normalized_weight", {})
        return {
            "best_strategy": best.get("strategy") if best else None,
            "best_strategy_reason": self.best_strategy_reason(best),
            "dynamic_allocation_improved_vs_s2": bool(improved_vs_s2),
            "dynamic_allocation_improved_vs_s2_strategies": improved_vs_s2,
            "dynamic_allocation_improved_vs_baseline": bool(improved_vs_baseline),
            "dynamic_allocation_improved_vs_baseline_strategies": improved_vs_baseline,
            "raw_vs_normalized_summary": self.raw_vs_normalized_summary(raw, normalized),
            "ready_for_phase12c": bool(improved_vs_baseline),
            "recommended_next_phase": "Phase12-C limited dynamic allocation refinement" if improved_vs_baseline else "Phase12-B2 allocation execution adjustment",
        }

    def passes_phase12b(self, row: dict[str, Any], baseline: dict[str, Any]) -> bool:
        return (
            (_safe_float(row.get("net_profit")) or -10**18) > (_safe_float(baseline.get("net_profit")) or 10**18)
    ...
    def best_strategy(self, rows: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        def sort_key(row: dict[str, Any]) -> tuple[float, float, float, float]:
            ...
            pass_bonus = 10**7 if pf >= 1.5 and dd >= -0.12 and net_profit > 0 else 0.0
    ...
    def raw_vs_normalized_summary(self, raw: dict[str, Any], normalized: dict[str, Any]) -> str:
        if not raw or not normalized:
            return "Dynamic raw or normalized strategy was not evaluated."
        return (
            f"raw net_profit={raw.get('net_profit'):.0f}, PF={raw.get('PF'):.4f}, DD={raw.get('DD'):.4f}, "
            f"utilization={raw.get('capital_utilization'):.4f}; normalized net_profit={normalized.get('net_profit'):.0f}, "
            f"PF={normalized.get('PF'):.4f}, DD={normalized.get('DD'):.4f}, utilization={normalized.get('capital_utilization'):.4f}."
        )
    ...
    def conditions(self) -> dict[str, Any]:
        return {
            ...
            "opportunity_exit_enabled_for": ["S2_opportunity_top5_E4", "S3a_dynamic_raw_weight", "S3b_dynamic_normalized_weight"],
    ...
    def recommendation(self, comparison: dict[str, Any], leakage: dict[str, Any]) -> dict[str, Any]:
        if leakage["blocking_issues"]:
            return {"ready_for_phase12c": False, "recommended_next_phase": "Fix Phase12-B leakage blockers"}
        return {
            "best_strategy": comparison.get("best_strategy"),
            "best_strategy_reason": comparison.get("best_strategy_reason"),
            "dynamic_allocation_improved_vs_s2": comparison.get("dynamic_allocation_improved_vs_s2"),
            "dynamic_allocation_improved_vs_baseline": comparison.get("dynamic_allocation_improved_vs_baseline"),
            "raw_vs_normalized_summary": comparison.get("raw_vs_normalized_summary"),
            "ready_for_phase12c": comparison.get("ready_for_phase12c"),
            "recommended_next_phase": comparison.get("recommended_next_phase"),
            "reason": "Forward only if S3a or S3b beats baseline with PF >= 1.5, DD >= -12%, and positive net profit under 0.2% one-way cost.",
        }

    def save_outputs(self, report: dict[str, Any]) -> Phase12BPaths:
        ...
        return Phase12BPaths(markdown=markdown_path, json=json_path)
    ...
    def to_markdown(self, report: dict[str, Any]) -> str:
        lines = [
            ...
            "",
            self.table(
                [report.get("strategy_comparison", {})],
                [
                    "best_strategy",
                    "best_strategy_reason",
                    "dynamic_allocation_improved_vs_s2",
                    "dynamic_allocation_improved_vs_baseline",
                    "raw_vs_normalized_summary",
                    "ready_for_phase12c",
            ...
            "## Strategy Results",
            "",
            self.table(report.get("strategy_results", []), ["strategy", "net_profit", "PF", "DD", "win_rate", "total_trades", "final_assets", "capital_utilization", "average_holding_days", "median_holding_days", "same_code_reentry_count", "reentry_within_5_days_count", "cost_paid", "exit_reason_counts"]),
            "",
            "## BUY Quality",
            "",
            self.table(report.get("buy_quality", []), ["strategy", "buy_count", "future_return_20d_mean", "future_max_return_20d_mean", "future_max_drawdown_20d_mean", "opportunity_value_20d_mean", "opportunity_top_decile_20d_rate", "downside_bad_rate", "average_allocation_weight"]),
            "",
            "## Leakage Checklist",
            "",
            ...
            "## Recommendation",
            "",
            self.table([report["recommendation"]], ["best_strategy", "dynamic_allocation_improved_vs_s2", "dynamic_allocation_improved_vs_baseline", "ready_for_phase12c", "recommended_next_phase", "reason"]),
            "",
        ]
        return "\n".join(lines)
