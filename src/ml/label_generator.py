...
class LabelGenerator:
    ...
    def generate_labels(self, target_date: str) -> pd.DataFrame:
        ...
        end_date = target + pd.Timedelta(days=self.lookahead_days)
        ...
        window_dates = set(business_days[1:11])
        ...
        window = prices[prices["date"].isin(window_dates)]
        extremes = window.groupby("code", as_index=False).agg(max_high_10d=("high", "max"), min_low_10d=("low", "min"))
    ...
    def update_available_labels(self, as_of_date: str) -> list[Path]:
        ...
        start_date = as_of - pd.Timedelta(days=self.lookahead_days * 2)
