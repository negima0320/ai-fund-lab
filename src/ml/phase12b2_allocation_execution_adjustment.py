"""Phase 12-B2 allocation execution adjustment.

This module keeps the Phase 12-A3 candidate universe and downside weight fixed,
then compares a small set of execution policies between raw and normalized
allocation. It is 2025-only and does not run a full backtest, change profiles,
...
from ml.phase12b_limited_allocation_strategy_check import (
    ARTIFACT_PATH,
...
REPORT_STEM = "phase12b2_allocation_execution_adjustment_2025"
FUTURE_EVAL_COLUMNS = EVAL_COLUMNS


@dataclass(frozen=True)
class Phase12B2Options:
...
@dataclass(frozen=True)
class Phase12B2Paths:
    markdown: Path
    json: Path


STRATEGIES = [
    StrategySpec("S0_baseline_equal_allocation", rank_column="baseline_rank_score", exit_guard=False),
    ...
    StrategySpec("S3b_dynamic_normalized_weight", rank_column="opportunity_proba", exit_guard=True, allocation_mode="dynamic_normalized"),
    StrategySpec("S4_partial_normalized_50", rank_column="opportunity_proba", exit_guard=True, allocation_mode="partial_50"),
    StrategySpec("S5_partial_normalized_30", rank_column="opportunity_proba", exit_guard=True, allocation_mode="partial_30"),
    StrategySpec("S6_min_usage_guard_30pct", rank_column="opportunity_proba", exit_guard=True, allocation_mode="min_usage_30"),
    StrategySpec("S7_min_usage_guard_50pct", rank_column="opportunity_proba", exit_guard=True, allocation_mode="min_usage_50"),
    StrategySpec("S8_capped_normalized", rank_column="opportunity_proba", exit_guard=True, allocation_mode="capped_300k"),
    StrategySpec("S9_capped_normalized_tighter", rank_column="opportunity_proba", exit_guard=True, allocation_mode="capped_200k"),
]


class Phase12B2AllocationExecutionAdjustment:
    def __init__(self, root: Path | str = ROOT, *, options: Phase12B2Options | None = None) -> None:
        self.root = Path(root)
        self.options = options or Phase12B2Options()

    def run(self) -> Phase12B2Paths:
            ...
            daily_rows.append(
                {
                    ...
                    "daily_budget_usage": budget_used_today / self.options.daily_buy_budget if self.options.daily_buy_budget else None,
    ...
    def target_amounts(self, selected: pd.DataFrame, spec: StrategySpec, cash: float) -> dict[Any, float]:
        ...
        if spec.allocation_mode == "min_usage_30":
            return self.min_usage_amounts(raw, normalized, min_usage=0.30, available_budget=available_budget)
        if spec.allocation_mode == "min_usage_50":
            return self.min_usage_amounts(raw, normalized, min_usage=0.50, available_budget=available_budget)
        if spec.allocation_mode == "capped_300k":
            return {index: min(amount, 300_000.0) for index, amount in normalized.items()}
        if spec.allocation_mode == "capped_200k":
            return {index: min(amount, 200_000.0) for index, amount in normalized.items()}
    ...
    def min_usage_amounts(self, raw: dict[Any, float], normalized: dict[Any, float], *, min_usage: float, available_budget: float) -> dict[Any, float]:
        raw_total = sum(raw.values())
        target_total = min(available_budget, self.options.daily_buy_budget * min_usage)
        if raw_total >= target_total:
            return raw
        normalized_total = sum(normalized.values())
        if normalized_total <= raw_total:
            return raw
        blend = min(1.0, (target_total - raw_total) / (normalized_total - raw_total))
        return self.blend_amounts(raw, normalized, normalized_share=blend)
    ...
    def metrics(self, strategy: str, trades: pd.DataFrame, daily: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            ...
            "average_daily_budget_usage": _safe_float(_numeric(daily["daily_budget_usage"]).mean()) if not daily.empty else None,
    ...
    def buy_quality(self, strategy: str, trades: pd.DataFrame) -> dict[str, Any]:
        ...
        weights = _numeric(trades["allocation_weight"]) if "allocation_weight" in trades.columns else pd.Series(dtype=float)
        targets = _numeric(trades["target_buy_amount"]) if "target_buy_amount" in trades.columns else pd.Series(dtype=float)
        row["average_allocation_weight"] = _safe_float(weights.mean()) if not weights.empty else None
        row["average_target_buy_amount"] = _safe_float(targets.mean()) if not targets.empty else None
        return row
    ...
    def strategy_comparison(self, rows: list[dict[str, Any]]) -> dict[str, Any]:
        minimum = [row for row in rows if self.minimum_passed(row)]
        ideal = [row for row in rows if self.ideal_passed(row)]
        ...
        return {
            ...
            "strategies_meeting_minimum_target": [row["strategy"] for row in minimum],
            "strategies_meeting_ideal_target": [row["strategy"] for row in ideal],
            "raw_vs_partial_vs_normalized_summary": self.raw_vs_partial_vs_normalized_summary(by_strategy),
            "ready_for_phase12c": bool(minimum),
            "recommended_next_phase": "Phase12-C limited dynamic allocation integration" if minimum else "Phase12-B3 execution adjustment or Phase12-A4 risk score refinement",
        }

    def minimum_passed(self, row: dict[str, Any]) -> bool:
        return (
            (_safe_float(row.get("net_profit")) or 0.0) > 0
    ...
    def ideal_passed(self, row: dict[str, Any]) -> bool:
        ...
        return (
            ...
            and (_safe_float(row.get("net_profit")) or 0.0) > baseline_profit
        )
    ...
    def best_strategy(self, rows: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        def sort_key(row: dict[str, Any]) -> tuple[float, float, float, float]:
            net_profit = _safe_float(row.get("net_profit")) or -10**18
            ...
            return (pass_bonus + net_profit, pf, dd, utilization)
    ...
    def best_strategy_reason(self, row: dict[str, Any] | None) -> str:
        ...
        return (
            ...
            f"average_daily_budget_usage={row.get('average_daily_budget_usage'):.4f}."
        )

    def raw_vs_partial_vs_normalized_summary(self, by_strategy: dict[str, dict[str, Any]]) -> str:
        names = [
            "S3a_dynamic_raw_weight",
            "S5_partial_normalized_30",
            "S4_partial_normalized_50",
        ...
        for name in names:
            ...
            if row:
                parts.append(f"{name}: profit={row.get('net_profit'):.0f}, PF={row.get('PF'):.4f}, DD={row.get('DD'):.4f}, util={row.get('capital_utilization'):.4f}")
        return "; ".join(parts)
    ...
    def conditions(self) -> dict[str, Any]:
        return {
            ...
            "opportunity_exit_enabled": True,
            "a3_3_weight_definition": "downside_rank <= .40:1.0, <=.70:.6, <=.85:.3, else:0.0",
    ...
    def leakage_checklist(self) -> dict[str, Any]:
        return {
            ...
            "full_backtest_executed": False,
            "decision_columns": ["stock_selection_rank_score", "opportunity_proba", "opportunity_rank_percentile", "downside_rank_percentile", "close", "turnover_value", "cash"],
            "leakage_risk": "low",
            "blocking_issues": [],
        }

    def recommendation(self, comparison: dict[str, Any], leakage: dict[str, Any]) -> dict[str, Any]:
        if leakage["blocking_issues"]:
            return {"ready_for_phase12c": False, "recommended_next_phase": "Fix Phase12-B2 leakage blockers"}
        return {
            "best_strategy": comparison.get("best_strategy"),
            "best_strategy_reason": comparison.get("best_strategy_reason"),
            "strategies_meeting_minimum_target": comparison.get("strategies_meeting_minimum_target", []),
            "strategies_meeting_ideal_target": comparison.get("strategies_meeting_ideal_target", []),
            "raw_vs_partial_vs_normalized_summary": comparison.get("raw_vs_partial_vs_normalized_summary"),
            "ready_for_phase12c": comparison.get("ready_for_phase12c", False),
            "recommended_next_phase": comparison.get("recommended_next_phase"),
            "reason": "Forward if at least one adjusted dynamic strategy has net_profit > 0, PF >= 1.5, DD >= -12%, and capital_utilization >= 0.20 under 0.2% one-way cost.",
        }

    def save_outputs(self, report: dict[str, Any]) -> Phase12B2Paths:
        ...
        return Phase12B2Paths(markdown=markdown_path, json=json_path)
    ...
    def to_markdown(self, report: dict[str, Any]) -> str:
        lines = [
            ...
            "## Summary",
            "",
            self.table(report.get("strategy_results", []), ["strategy", "net_profit", "PF", "DD", "win_rate", "total_trades", "final_assets", "capital_utilization", "average_daily_budget_usage", "average_holding_days", "median_holding_days", "same_code_reentry_count", "reentry_within_5_days_count", "cost_paid", "exit_reason_counts"]),
            "",
            "## BUY Quality",
            "",
            self.table(report.get("buy_quality", []), ["strategy", "buy_count", "future_return_20d_mean", "future_max_return_20d_mean", "future_max_drawdown_20d_mean", "opportunity_value_20d_mean", "opportunity_top_decile_20d_rate", "downside_bad_rate", "average_allocation_weight", "average_target_buy_amount"]),
            "",
            "## Strategy Comparison",
            "",
            self.table([report.get("strategy_comparison", {})], ["best_strategy", "best_strategy_reason", "strategies_meeting_minimum_target", "strategies_meeting_ideal_target", "raw_vs_partial_vs_normalized_summary", "ready_for_phase12c", "recommended_next_phase"]),
            "",
            "## Leakage Checklist",
            "",
            ...
            "## Recommendation",
            "",
            self.table([report["recommendation"]], ["best_strategy", "strategies_meeting_minimum_target", "strategies_meeting_ideal_target", "ready_for_phase12c", "recommended_next_phase", "reason"]),
            "",
        ]
        return "\n".join(lines)
