...
from ml.phase12c4_concentration_guard_refinement import Phase12C4ConcentrationGuardRefinement


EXPECTED_VARIANTS = {
    "C4_0_baseline_downside_squared",
    "C4_1_cap_40_redistribute",
    "C4_2_cap_35_redistribute",
    "C4_3_cap_30_redistribute",
    "C4_4_dynamic_cap_by_score_gap",
    "C4_5_staged_buy_half_first",
    "C4_6_staged_buy_70pct_first",
}
...
def test_phase12c4_builds_refinement_report(tmp_path: Path) -> None:
    _write_artifact(tmp_path)

    report = Phase12C4ConcentrationGuardRefinement(tmp_path).build_report()

    assert report["metadata"]["phase"] == "12-C4"
    ...
    assert "cap_redistribution_effect" in report["recommendation"]
    assert "dynamic_cap_effect" in report["recommendation"]
    assert "staged_buy_effect" in report["recommendation"]
    assert "ready_for_phase13" in report["recommendation"]


def test_phase12c4_saves_reports(tmp_path: Path) -> None:
    _write_artifact(tmp_path)

    paths = Phase12C4ConcentrationGuardRefinement(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "12-C4"
