...
from ml.phase12a_dynamic_capital_allocation import Phase12ADynamicCapitalAllocation, Phase12AOptions
...
def _write_fixture(root: Path) -> None:
    ...
    for year in [2023, 2024, 2025]:
        for date in pd.bdate_range(f"{year}-01-04", periods=35):
            for rank in range(12):
                opportunity = (11 - rank) / 11
                downside = rank in {0, 1, 10, 11}
                rows.append(
                    {
                        "date": date,
                        "code": f"{10000 + rank}",
                        ...
                        "relative_return_20d": 0.25 if downside else opportunity / 25,
                        "ma25_gap": opportunity / 40,
                        "ma75_gap": 0.20 if downside else opportunity / 60,
                        "daily_range_ratio": 0.08 if downside else 0.02,
                        "volume": 100_000 + rank,
                        ...
                        "future_return_20d": 0.06 - rank * 0.004 - (0.04 if downside else 0.0),
                        "future_max_return_20d": 0.15 - rank * 0.005,
                        "future_max_drawdown_20d": -0.14 if downside else -0.04 - rank * 0.002,
...
def _prepare_b3_models(root: Path) -> None:
    Phase11B3ExpectedDownsideModel(root, options=Phase11B3Options(max_train_rows=500, max_iter=20)).run()


def test_phase12a_builds_allocation_quality_report(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    _prepare_b3_models(tmp_path)

    report, allocation_rows = Phase12ADynamicCapitalAllocation(tmp_path, options=Phase12AOptions(save_artifact=False)).build_report()
    ...
    assert {row["allocation_rule"] for row in report["rule_quality"]} == {
...
def test_phase12a_saves_reports_and_optional_artifact(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    _prepare_b3_models(tmp_path)

    paths = Phase12ADynamicCapitalAllocation(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    assert paths.artifact is not None
    assert paths.artifact.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "12-A"
