...
from ml.phase11h_cooldown_minhold_guard import Phase11HCooldownMinHoldGuard
...
def _write_fixture(root: Path) -> None:
    ...
    for year in [2024, 2025]:
        for day_index, date in enumerate(pd.bdate_range(f"{year}-01-04", periods=40)):
            for rank in range(8):
                quality = (7 - rank) / 7
                code = f"{10000 + rank}"
                close = 100 + rank * 4
...
def test_phase11h_builds_guard_report(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    report = Phase11HCooldownMinHoldGuard(tmp_path).build_report()

    assert report["metadata"]["phase"] == "11-H"
    assert report["metadata"]["walk_forward_retraining_executed"] is False
    assert report["leakage_checklist"]["leakage_risk"] == "low"
    assert report["leakage_checklist"]["blocking_issues"] == []
    assert {row["year"] for row in report["variant_results"]} == {2024, 2025}
    assert {row["variant"] for row in report["variant_results"]} >= {
        "H0_baseline_E4",
        "H1_cooldown_5d",
        "H2_cooldown_10d",
        "H3_min_hold_3d",
        "H4_cooldown_5d_min_hold_3d",
    }
    assert report["strict_walk_forward_oos_design"]["phase11h_retraining_executed"] is False
    assert "recommended_first_split" in report["strict_walk_forward_oos_design"]["required_retraining_splits"]


def test_phase11h_saves_reports(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    paths = Phase11HCooldownMinHoldGuard(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "11-H"
