...
from ml.phase12b2_allocation_execution_adjustment import Phase12B2AllocationExecutionAdjustment
...
EXPECTED_STRATEGIES = {
    ...
    "S6_min_usage_guard_30pct",
    "S7_min_usage_guard_50pct",
    "S8_capped_normalized",
    "S9_capped_normalized_tighter",
}
...
def test_phase12b2_builds_execution_adjustment_report(tmp_path: Path) -> None:
    _write_artifact(tmp_path)

    report = Phase12B2AllocationExecutionAdjustment(tmp_path).build_report()

    assert report["metadata"]["phase"] == "12-B2"
    ...
    assert "strategies_meeting_minimum_target" in report["strategy_comparison"]


def test_phase12b2_saves_reports(tmp_path: Path) -> None:
    _write_artifact(tmp_path)

    paths = Phase12B2AllocationExecutionAdjustment(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "12-B2"
