...
def test_selection_quality_compares_selected_and_rejected_future_returns(config_copy: dict, tmp_path) -> None:
    ...
    assert analysis["screen_candidate_count"] == 4
    assert analysis["score_candidate_count"] == 3
    assert analysis["selected_count"] == 2
    assert analysis["rejected_count"] == 1
    assert analysis["selected"]["average_return_5d"] == -0.025
    assert analysis["rejected"]["average_return_5d"] == 0.2
    assert analysis["selected"]["average_return_10d"] == -0.05
    assert analysis["rejected"]["average_return_10d"] == 0.3
    assert analysis["selection_lift"]["return_5d"] == -0.225
    assert analysis["selection_lift"]["return_10d"] == -0.35
