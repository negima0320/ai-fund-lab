...
from ml.phase13d2_hold_exit_label_refinement import REQUIRED_REPORT_KEYS, Phase13D2HoldExitLabelRefinement
...
def test_phase13d2_builds_required_report(tmp_path: Path) -> None:
    _write_artifacts(tmp_path)

    report = Phase13D2HoldExitLabelRefinement(tmp_path).build_report()

    assert report["metadata"]["phase"] == "13-D2"
    ...
    assert report["exclusive_label_definitions"]
    assert report["exclusive_label_distribution"]
    assert report["recommended_label_priority"]
...
def test_phase13d2_saves_json_report(tmp_path: Path) -> None:
    _write_artifacts(tmp_path)

    paths = Phase13D2HoldExitLabelRefinement(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "13-D2"
    assert loaded["primary_candidate_set"] == "candidate_strength_top50"
