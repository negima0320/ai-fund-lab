...
EXPECTED_VARIANTS = {
    "C3_0_baseline_downside_squared",
    "C3_1_per_name_cap_40pct",
    "C3_2_per_name_cap_30pct",
    "C3_3_per_name_cap_25pct",
    "C3_4_top2_cap_60pct",
    "C3_5_per_name_30pct_and_top2_60pct",
    "C3_6_concentration_scaled",
}
...
def test_phase12c3_builds_concentration_guard_report(tmp_path: Path) -> None:
    _write_artifact(tmp_path)

    report = Phase12C3PositionConcentrationGuard(tmp_path).build_report()

    assert report["metadata"]["phase"] == "12-C3"
    ...
    assert "largest_position_weight_max" in report["variant_results"][0]
    assert "concentration_reduction_summary" in report["recommendation"]
    assert "ready_for_phase13" in report["recommendation"]


def test_phase12c3_saves_reports(tmp_path: Path) -> None:
    _write_artifact(tmp_path)

    paths = Phase12C3PositionConcentrationGuard(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "12-C3"
