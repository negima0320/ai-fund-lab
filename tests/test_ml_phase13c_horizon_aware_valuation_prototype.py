...
from ml.phase13c_horizon_aware_valuation_prototype import REQUIRED_REPORT_KEYS, Phase13CHorizonAwareValuationPrototype
...
def test_phase13c_builds_required_report(tmp_path: Path) -> None:
    _write_artifacts(tmp_path)

    report = Phase13CHorizonAwareValuationPrototype(tmp_path).build_report()

    assert report["metadata"]["phase"] == "13-C"
    ...
    assert report["leakage_checklist"]["future_columns_used_as_scores"] == []
    ...
    assert report["recommended_valuation_score"]
    assert report["expected_mean_trade_return_20d"] is not None
    assert report["required_mean_trade_return_20d"] is not None
    assert "meets_required_trade_return" in report
...
def test_phase13c_saves_json_report(tmp_path: Path) -> None:
    _write_artifacts(tmp_path)

    paths = Phase13CHorizonAwareValuationPrototype(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "13-C"
    assert loaded["leakage_checklist"]["historical_predictions_regenerated"] is False
    assert loaded["leakage_checklist"]["future_columns_used_as_scores"] == []
