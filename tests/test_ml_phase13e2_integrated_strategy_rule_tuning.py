...
from ml.phase13e2_integrated_strategy_rule_tuning import COST_RATE, REQUIRED_REPORT_KEYS, VARIANTS, Phase13E2IntegratedStrategyRuleTuning
...
def test_phase13e2_builds_required_report(tmp_path: Path) -> None:
    _write_artifacts(tmp_path)

    report = Phase13E2IntegratedStrategyRuleTuning(tmp_path).build_report()

    assert report["metadata"]["phase"] == "13-E2"
    ...
    for config in report["variant_definitions"][:8]:
        assert config["candidate_top_n"] == 50
    ...
    assert "improvement_vs_phase13e_e3" in report
...
def test_phase13e2_saves_json_report(tmp_path: Path) -> None:
    _write_artifacts(tmp_path)

    paths = Phase13E2IntegratedStrategyRuleTuning(tmp_path).run()

    assert paths.markdown.exists()
    assert paths.json.exists()
    loaded = json.loads(paths.json.read_text(encoding="utf-8"))
    assert loaded["metadata"]["phase"] == "13-E2"
