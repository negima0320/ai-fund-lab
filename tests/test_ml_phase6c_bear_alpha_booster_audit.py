...
def _write_fixture(root: Path) -> None:
    ...
    pd.DataFrame(
        [
            ...
            {"code": "44440", "entry_date": dates[74].strftime("%Y-%m-%d"), "final_amount": 10_000},
        ]
...
def test_phase6c_builds_bear_alpha_booster_audit(tmp_path: Path) -> None:
    _write_fixture(tmp_path)

    result = Phase6CBearAlphaBoosterAudit(tmp_path).build_report()

    assert result["metadata"]["audit_only"] is True
    assert result["metadata"]["full_backtest_executed"] is False
    assert result["coverage"]["bear_trades"] >= 3
    conditions = {row["condition"]: row for row in result["bear_alpha_condition_audit"]}
    assert conditions["D_Bear_PM_1_30_volume_ratio_gte_2"]["trade_count"] >= 1
    assert result["booster_virtual_audit"]
    assert result["pm_080_analysis"]["summary"]["trade_count"] >= 1
    assert "bear_alpha_exists" in result["verdict"]


def test_phase6c_saves_reports(tmp_path: Path) -> None:
    _write_fixture(tmp_path)
    audit = Phase6CBearAlphaBoosterAudit(tmp_path)

    paths = audit.save_report(audit.build_report())

    assert paths.markdown.exists()
    assert paths.json.exists()
    assert "Phase 6-C" in paths.markdown.read_text(encoding="utf-8")
    payload = json.loads(paths.json.read_text(encoding="utf-8"))
    assert payload["sources"]["regime_note"].startswith("TOPIX rows")
