...
この構成では `OPENAI_API_KEY` が未設定でも実行できます。候補銘柄の最終判断とコメント生成はルールベースで完結し、backtest中はGoogle News RSS取得も行いません。実売買は行いません。J-Quants Freeプランでは12週間遅延データを前提に検証します。

`fetch-period-prices` では、対象期間、キャッシュ済み日数、未取得日数、取得中の日付をコンソールへ表示します。`data/raw/prices_YYYY-MM-DD.json` がある日はAPIを呼ばずキャッシュを使います。J-Quants APIの通信には `config/rookie_dealer.yaml` の `jquants.request_timeout_seconds` を使い、期間取得に失敗した場合も既存キャッシュからバックテスト可能な日だけで続行します。停止したい場合は `Ctrl+C` で安全に中断できます。

各dayでは `calculate-indicators`、`market-context`、`screen`、`score`、`ai-decision`、`trade`、`reports/articles`、`db-save` の開始/終了と処理時間を表示します。`indicators input rows`、`screen target stocks`、`scoring candidates` などの件数もあわせて確認できます。

`backtest.indicator_mode` は `full`、`fast`、`minimal` から選べます。デフォルトは `fast` です。`full` はpandas-taを含む全指標、`fast` は移動平均線、RSI、出来高倍率、売買代金、5日ボラティリティ、ローソク足特徴量、`minimal` は移動平均線、RSI、出来高倍率のみを計算します。backtest中は `data/processed/<profile_id>/indicators_YYYY-MM-DD.json` に同じモードのキャッシュがあれば再計算しません。
