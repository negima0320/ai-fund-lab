...
## Phase 11-I Implementation Status
...
- research-only strict split model
- classification only: `opportunity_top_decile_20d`
- train `2023-01-04` to `2023-12-31`
- validation `2024-01-01` to `2024-12-31`
- test `2025-01-01` to `2025-12-31`
- 2026年は未使用
- Phase 11-B candidate model上書きなし
- profile追加/変更なし
- full period backtestなし
- historical prediction再生成なし
- future系は評価指標のみ
...
- strict splitでもtop decile検出liftは残った。
- ただしPhase 11-Bの2025 holdoutよりAUC / PR-AUC / precision@top10%は低下した。
- 2023単年trainでは、2024を含むPhase 11-B modelほど強いrank qualityを再現できていない。

2025 strategy check with `0.2%` one-way cost:

| strategy | net_profit | PF | DD | trades | avg_hold | reentry_5d |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `baseline_equal_allocation` | `180,876` | `2.2930` | `-6.39%` | `55` | `19.93` | `22` |
| `strict_oos_valuation_top5_no_guard` | `-85,275` | `0.8169` | `-22.35%` | `60` | `19.70` | `26` |
| `strict_oos_E4` | `116,049` | `1.2501` | `-13.14%` | `141` | `7.67` | `74` |
| `strict_oos_H2_cooldown_10d` | `-75,507` | `0.8230` | `-14.27%` | `114` | `8.45` | `37` |
| `strict_oos_H3_min_hold_3d` | `42,411` | `1.0923` | `-15.26%` | `129` | `8.71` | `67` |

BUY quality:

| strategy | future_return_20d | future_max_return_20d | future_max_drawdown_20d | opportunity_value_20d | top_decile_rate |
| --- | ---: | ---: | ---: | ---: | ---: |
| `baseline_equal_allocation` | `0.0312` | `0.0900` | `-0.0469` | `0.0431` | `0.1818` |
| `strict_oos_valuation_top5_no_guard` | `0.0139` | `0.1485` | `-0.0915` | `0.0570` | `0.3000` |
| `strict_oos_E4` | `0.0354` | `0.1389` | `-0.0879` | `0.0510` | `0.2553` |
| `strict_oos_H2_cooldown_10d` | `0.0212` | `0.1284` | `-0.0889` | `0.0396` | `0.2193` |
| `strict_oos_H3_min_hold_3d` | `0.0308` | `0.1421` | `-0.0925` | `0.0496` | `0.2481` |

Strict OOS judgement:

| check | result |
| --- | --- |
| strict OOS strategy beats baseline with PF >= `1.5` | `false` |
| DD >= `-12%` with positive net profit | `false` |
| cost `0.2%`込みで合格 | `false` |
| better conditions passed | `false` |

Interpretation:

- strict OOS modelでは、Valuation rank自体はpositive classを濃縮したが、2025 strategy checkではbaselineを超えられなかった。
- Phase 11-B/11-F/11-Hで見えたE4/H2/H3の強さは、2023-only train strict modelでは再現不足。
- 2025 baselineがPhase 11-D時より強く見えるのは、Phase 11-Iでは全strategyを `0.2%` cost込みの同一simulation実装で比較しているため、過去Phaseの数値とは同列比較しない。
...
| future_columns_used_only_for_evaluation | `future_return_20d`, `future_max_return_20d`, `future_max_drawdown_20d`, `opportunity_value_20d`, `opportunity_top_decile_20d` |
...
- `strict_oos_effective`: `false`
- `recommended_next_phase`: `Phase11-B2 valuation model improvement`
