...
from ml.phase5c_exit_ai_v2_dataset_builder import BuildOptions, Phase5CExitAIV2DatasetBuilder  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or audit the Exit AI v2 API-only dataset.")
    parser.add_argument("--dry-run", action="store_true", help="Generate report only. This is the safe default.")
    parser.add_argument("--sample-rows", type=int, default=None, help="Write only the first N post-filter rows.")
...
def main() -> int:
    ...
    options = BuildOptions(
        dry_run=args.dry_run or (not args.write_full and not args.sample_rows),
    ...
    print(f"rows_after_horizon_filter={result['row_counts']['rows_after_horizon_filter']}")
